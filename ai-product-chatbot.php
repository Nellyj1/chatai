...
class AI_Product_Chatbot {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_aipc_get_faq_list', [$this, 'handle_get_faq_list']);
        add_action('wp_ajax_nopriv_aipc_get_faq_list', [$this, 'handle_get_faq_list']);
    ...
    public function enqueue_scripts() {
        ...
        // Always add CSS variables for dynamic colors
    ...
    private function add_css_variables() {
        ...
        $quick_action_primary = get_option('aipc_quick_action_primary_color', '#6366f1');
        $quick_action_secondary = get_option('aipc_quick_action_secondary_color', '#8b5cf6');
        
        echo "<style id='aipc-css-variables'>
        :root {
            --aipc-quick-action-primary: {$quick_action_primary};
            --aipc-quick-action-secondary: {$quick_action_secondary};
        }
        </style>";
    }
    ...
    private function add_custom_css() {
        ...
        /* Quick Action Buttons (FAQ/Quiz) */
        .aipc-quick-btn {
            background: linear-gradient(135deg, {$quick_action_primary}, {$quick_action_secondary}) !important;
        }
        
        .aipc-quick-btn:hover {
            box-shadow: 0 4px 12px rgba(". $this->hex_to_rgba($quick_action_primary, 0.4) .") !important;
        }
        
        /* FAQ option buttons */
        .aipc-faq-option:hover {
            box-shadow: 0 4px 8px rgba(". $this->hex_to_rgba($quick_action_primary, 0.1) .") !important;
        }
        </style>
        ";
    ...
    public function register_settings() {
        ...
        // Quick Action Button colors
        register_setting('aipc_settings', 'aipc_quick_action_primary_color', [
            'type' => 'string',
            'sanitize_callback' => 'sanitize_hex_color',
            'default' => '#6366f1'
        ]);
        register_setting('aipc_settings', 'aipc_quick_action_secondary_color', [
            'type' => 'string',
            'sanitize_callback' => 'sanitize_hex_color',
            'default' => '#8b5cf6'
        ]);
    ...
    public function handle_get_faq_list() {
        check_ajax_referer('aipc_nonce', 'nonce');
        ...
        try {
            global $wpdb;
            $table_name = $wpdb->prefix . 'aipc_faq';
            
            // Check if FAQ table exists
            if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name) {
                wp_send_json_error([
                    'message' => $is_english ? 'FAQ database not found.' : 'FAQ database niet gevonden.'
                ]);
            }
            
            // Get FAQ items
            $faqs = $wpdb->get_results(
                "SELECT id, question, answer FROM {$table_name} ORDER BY question ASC",
                ARRAY_A
            );
            ...
            if (empty($faqs)) {
                wp_send_json_success([
                    'faqs' => [],
                    'message' => $is_english ? 'No FAQ items found.' : 'Geen FAQ items gevonden.'
                ]);
            }
            
            // Format FAQs for frontend
            $formatted_faqs = [];
            foreach ($faqs as $faq) {
                $formatted_faqs[] = [
                    'id' => intval($faq['id']),
            ...
            wp_send_json_error([
                'message' => $is_english ? 'Error retrieving FAQ data.' : 'Fout bij ophalen van FAQ gegevens.'
            ]);
        }
    }
